...
var SVG = (function () {
            ...
            if (typeof style === "object") {
                for (const name in style) {
                    ...
                    this.node.style[camelCase(name)] =
                        style[name] == null || isBlank.test(style[name])
                            ? ""
                            : style[name];
                }
            }
    ...
    */
        ...
        init(a = 0, b = 0, c = 0, d = 0, space = "rgb") {
            ...
            if (typeof a === "number") {
                ...
                this.space =
                    b || (typeof a[3] === "string" ? a[3] : a[4]) || "rgb";
                Object.assign(this, {
                    _a: a[0],
                    _b: a[1],
                    _c: a[2],
    ...
    */
        ...
        rgb() {
            if (this.space === "rgb") {
                ...
                const bd = 0.0031308;
                const r =
                    rU > bd ? 1.055 * pow(rU, 1 / 2.4) - 0.055 : 12.92 * rU;
                const g =
                    gU > bd ? 1.055 * pow(gU, 1 / 2.4) - 0.055 : 12.92 * gU;
                const b =
                    bU > bd ? 1.055 * pow(bU, 1 / 2.4) - 0.055 : 12.92 * bU; // Make and return the color
        ...
        xyz() {
            ...
            const bL =
                b > 0.04045 ? Math.pow((b + 0.055) / 1.055, 2.4) : b / 12.92; // Convert to the xyz color space without bounding the values
            ...
            const x =
                xU > 0.008856 ? Math.pow(xU, 1 / 3) : 7.787 * xU + 16 / 116;
            const y =
                yU > 0.008856 ? Math.pow(yU, 1 / 3) : 7.787 * yU + 16 / 116;
            const z =
                zU > 0.008856 ? Math.pow(zU, 1 / 3) : 7.787 * zU + 16 / 116; // Make and return the color
    ...
    */
    ...
    function baseFind(query, parent) {
        return new List(
            map(
                (parent || globals.document).querySelectorAll(query),
                function (node) {
                    return adopt(node);
                }
            )
    ...
    class Element extends Dom {
        ...
        size(width, height) {
            ...
            return this.width(new SVGNumber(p.width)).height(
    ...
    class Container extends Element {
        ...
        ungroup(parent = this.parent(), index = parent.index(this)) {
            ...
            this.each(function (i, children) {
                // reverse each
                return children[children.length - i - 1].toParent(
                    parent,
                    index
                );
            });
    ...
    class PointArray extends SVGArray {
        ...
        size(width, height) {
            ...
            for (i = this.length - 1; i >= 0; i--) {
                if (box.width)
                    this[i][0] =
                        ((this[i][0] - box.x) * width) / box.width + box.x;
                if (box.height)
                    this[i][1] =
                        ((this[i][1] - box.y) * height) / box.height + box.y;
            }
  ...
  ***/
    ...
    class Spring extends Controller {
        ...
        step(current, target, dt, c) {
            ...
            const acceleration =
                -this.d * velocity - this.k * (current - target);
            const newPosition =
                current + velocity * dt + (acceleration * dt * dt) / 2; // Store the velocity

            c.velocity = velocity + acceleration * dt; // Figure out if we have converged, and if so, pass the value

            c.done =
                Math.abs(target - newPosition) + Math.abs(velocity) < 0.002;
    ...
    class PID extends Controller {
        ...
        step(current, target, dt, c) {
            ...
            return c.done
                ? target
                : current + (this.P * p + this.I * i + this.D * d);
        }
    }
    extend(PID, {
    ...
    function makeAbsolut(parser) {
        ...
        return pathHandlers[command](
            parser.segment.slice(1),
            parser.p,
            parser.p0
        );
    }
    ...
    class PathArray extends SVGArray {
        ...
        size(width, height) {
            ...
            for (i = this.length - 1; i >= 0; i--) {
                ...
                if (l === "M" || l === "L" || l === "T") {
                    this[i][1] =
                        ((this[i][1] - box.x) * width) / box.width + box.x;
                    this[i][2] =
                        ((this[i][2] - box.y) * height) / box.height + box.y;
                } else if (l === "H") {
                    this[i][1] =
                        ((this[i][1] - box.x) * width) / box.width + box.x;
                } else if (l === "V") {
                    ...
                    this[i][2] =
                        ((this[i][2] - box.y) * height) / box.height + box.y;
                    this[i][3] =
                        ((this[i][3] - box.x) * width) / box.width + box.x;
                    this[i][4] =
                        ((this[i][4] - box.y) * height) / box.height + box.y;

                    if (l === "C") {
                        this[i][5] =
                            ((this[i][5] - box.x) * width) / box.width + box.x;
                        this[i][6] =
                            ((this[i][6] - box.y) * height) / box.height +
                            box.y;
                    }
                } else if (l === "A") {
                    // resize radii
                    ...
                    this[i][6] =
                        ((this[i][6] - box.x) * width) / box.width + box.x;
                    this[i][7] =
                        ((this[i][7] - box.y) * height) / box.height + box.y;
                }
            }
        ...
        at(pos) {
            ...
            return this._morphObj.fromArray(
                this._from.map(function (i, index) {
                    return _this._stepper.step(
                        i,
                        _this._to[index],
                        pos,
                        _this._context[index],
                        _this._context
                    );
                })
            );
    ...
    function array() {
        return (
            this._array || (this._array = new PointArray(this.attr("points")))
        );
    } // Clear array cache
    ...
    const Animator = {
        ...
        frame(fn) {
            ...
            if (Animator.nextDraw === null) {
                Animator.nextDraw = globals.window.requestAnimationFrame(
                    Animator._draw
                );
        ...
        _draw(now) {
            ...
            while (
                nextFrame !== lastFrame &&
                (nextFrame = Animator.frames.shift())
            ) {
                nextFrame.run(now);
            }
    ...
    class Timeline extends EventTarget {
        ...
        getEndTime() {
            ...
            const lastDuration = lastRunnerInfo
                ? lastRunnerInfo.runner.duration()
                : 0;
            const lastStartTime = lastRunnerInfo
                ? lastRunnerInfo.start
                : this._time;
            return lastStartTime + lastDuration;
        }

        getEndTimeOfTimeline() {
            const endTimes = this._runners.map(
                (i) => i.start + i.runner.duration()
            );
        ...
        _stepFn(immediateStep = false) {
            ...
            const dtTime =
                this._speed * dtSource + (this._time - this._lastStepTime);
            ...
            for (let i = 0, len = this._runners.length; i < len; i++) {
                ...
                if (!finished) {
                    ...
                    const endTime =
                        runner.duration() - runner.time() + this._time;
        ...
        constructor(options) {
            ...
            options =
                typeof options === "function"
                    ...
                    : options; // Declare all of the variables
    ...
    */
        ...
        */
                ...
                const f = function (x) {
                    const swinging =
                        s * Math.floor((x % (2 * (w + d))) / (w + d));
                    const backwards = (swinging && !r) || (!swinging && r);
                    const uncliped =
                        (Math.pow(-1, backwards) * (x % (w + d))) / d +
                        backwards;
    ...
    */
        ...
        step(dt) {
            ...
            const justFinished =
                this._lastTime < duration && this._time >= duration;
    ...
    */
        ...
        _initialise(running) {
            ...
            for (let i = 0, len = this._queue.length; i < len; ++i) {
                ...
                const needsIt =
                    this._isDeclarative || (!current.initialised && running);
        ...
        _tryRetarget(method, target, extra) {
            if (this._history[method]) {
                ...
                if (!this._history[method].caller.initialised) {
                    const index = this._queue.indexOf(
                        this._history[method].caller
                    );
                ...
                if (this._history[method].caller.retarget) {
                    this._history[method].caller.retarget.call(
                        this,
                        target,
                        extra
            ...
            for (let i = 0; i < this.runners.length; ++i) {
                ...
                const condition =
                    ...
                    (!lastRunner._timeline ||
                        !lastRunner._timeline._runnerIds.includes(
                            lastRunner.id
                        ));
    ...
    extend(Runner, {
            ...
            this.queue(
                ...
                function (newToAttrs) {
                    ...
                    if (differences.length) {
                        // Get the values
                        const addedFromAttrs =
                            this.element()[type](differences); // Get the already initialized values

                        const oldFromAttrs = new ObjectBag(
                            morpher.from()
                        ).valueOf(); // Merge old and new
        ...
        transform(transforms, relative, affine) {
            ...
            function run(pos) {
                ...
                let start =
                    this._isDeclarative && current ? current : startTransform;
                ...
                if (affine) {
                    ...
                    const distances = possibilities.map((a) =>
                        Math.abs(a - rCurrent)
                    );
    ...
    class Svg extends Container {
        ...
        defs() {
            ...
            return (
                adopt(this.node.querySelector("defs")) || this.put(new Defs())
            );
        }
            ...
            clipWith(element) {
                ...
                const clipper =
                    element instanceof ClipPath
                        ? element
                        : this.parent().clip().add(element); // apply mask
            ...
            maskWith(element) {
                ...
                const masker =
                    element instanceof Mask
                        ? element
                        : this.parent().mask().add(element); // apply mask
...
function Example() {
    ...
    useEffect(() => {
        ...
        const wheelCanvas = draw.group().flip("y", height / 2);
        const wheel = wheelCanvas.group();
        wheel
            .circle(100, 100)
            .center(185, 110)
            .fill("white")
        ...
        const wheelStartX = absorberEnd.cx();
        const wheelStartY = absorberEnd.cy();
        const chassisStartX = absorberStart.cx();
        const chassisStartY = absorberStart.cy();
    ...
    return (
        <div className="grid grid-cols-2 gap-4">
            <div className="w-full bg-white max-w-lg mx-auto p-8 md:p-12 my-10 rounded-lg shadow-2xl">
                <section>
                    <h3 className="font-bold text-2xl text-gray-600">
                        Animácia tlmičov vozidla
                    </h3>
                </section>
            ...
            <div>
                <div className="w-full bg-white max-w-lg mx-auto p-8 md:p-12 my-10 rounded-lg shadow-2xl mb-4">
                    ...
                    <section className="mt-10">
                        ...
                        >
                            <div className="mb-6 pt-3 rounded bg-gray-200">
                                <div className="mb-3 flex items-center justify-center">
                                    <h5 className="text-gray-600">Výsledok</h5>
                                </div>
                            </div>
                            <div className="mb-6 pt-3 rounded bg-gray-200">
                                <label
                ...
                <div className="w-full bg-white max-w-lg mx-auto p-8 md:p-12 my-10 rounded-lg shadow-2xl mt-4">
                    <section>
                        ...
                        <p className="text-gray-600 pt-2">Pridaj sa k nim.</p>
                    </section>
